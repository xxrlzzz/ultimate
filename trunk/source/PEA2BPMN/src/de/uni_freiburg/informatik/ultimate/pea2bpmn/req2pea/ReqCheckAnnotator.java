...
import de.uni_freiburg.informatik.ultimate.pea2bpmn.results.ReqCheck;
...
import de.uni_freiburg.informatik.ultimate.pea2bpmn.PeaResultUtil;
...
import de.uni_freiburg.informatik.ultimate.pea2bpmn.translator.CheckedReqLocation;
...
public class ReqCheckAnnotator implements IReq2PeaAnnotator {
	...
	public List<Statement> getStateChecks() {
		...
		// set preferences
		mCheckVacuity = prefs.getBoolean(Pea2BPMNPreferences.LABEL_CHECK_VACUITY);

		if (prefs.getBoolean(Pea2BPMNPreferences.LABEL_CHECK_RT_INCONSISTENCY)) {
			final int length = mReqPeas.size();
			mCombinationNum = Math.min(length, prefs.getInt(Pea2BPMNPreferences.LABEL_RT_INCONSISTENCY_RANGE));
		} else {
			mCombinationNum = -1;
		}
		mCheckConsistency = prefs.getBoolean(Pea2BPMNPreferences.LABEL_CHECK_CONSISTENCY);
		mReportTrivialConsistency = prefs.getBoolean(Pea2BPMNPreferences.LABEL_REPORT_TRIVIAL_RT_CONSISTENCY);
		mSeparateInvariantHandling = prefs.getBoolean(Pea2BPMNPreferences.LABEL_RT_INCONSISTENCY_USE_ALL_INVARIANTS);

		// log preferences
		mLogger.info(String.format("%s=%s, %s=%s, %s=%s, %s=%s, %s=%s", Pea2BPMNPreferences.LABEL_CHECK_VACUITY,
				mCheckVacuity, Pea2BPMNPreferences.LABEL_RT_INCONSISTENCY_RANGE, mCombinationNum,
				Pea2BPMNPreferences.LABEL_CHECK_CONSISTENCY, mCheckConsistency,
				Pea2BPMNPreferences.LABEL_REPORT_TRIVIAL_RT_CONSISTENCY, mReportTrivialConsistency,
				Pea2BPMNPreferences.LABEL_RT_INCONSISTENCY_USE_ALL_INVARIANTS, mSeparateInvariantHandling));
