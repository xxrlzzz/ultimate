...
public class TriggerResponseBoundL1PeaImpl extends AbsPeaImpl<TriggerResponseBoundL1Pattern> {
    ...
    public PEAFragment generate() {
        ...
        pr.addTransition(prs);
        prs.addTransition(pr);
        prst.addTransition(pst);
        p_true.addSelfTrans();

        pr.addTransition(pr, rtLess, new String[]{});
        prs.addTransition(prs, rtLess, new String[]{});

        pst.addTransition(p_true);
        prst.addTransition(p_true);
        ...
        pea.setDesc(new ReqDesc(mReq, List.of(R, S), List.of(T), rtLess, CDD.TRUE, CDD.TRUE, phase));
        return pea;
    }
}
